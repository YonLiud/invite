...
describe('AuthService', () => {
  ...
  describe('login', () => {
    beforeEach(async () => {
      ...
      await authService.register('testuser_' + Date.now(), 'password123', 'Test User');
    });
    ...
    it('should return null for invalid password', async () => {
      // Register a user first
      await authService.register('invalid_pass_user', 'password123', 'Test User');
      
      const result = await authService.login('invalid_pass_user', 'wrongpassword');
      expect(result).toBeNull();
    });
