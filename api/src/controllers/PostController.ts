...
export class PostController extends BaseController {
    ...
    async createPost(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (content.length > 1000) {
                this.sendValidationError(res, ['Content exceeds maximum length of 1000 characters']);
                return;
            }
    ...
    async getAllPosts(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (pageNum < 1 || limitNum < 1) {
                return this.sendValidationError(res, ['Page and limit must be positive integers']);
            }
    ...
    async getPostsByAuthor(req: Request, res: Response): Promise<void> {
        try {
            ...
            const posts = await this.postService.findByAuthorUsername(username, limitNum);
    ...
    async updatePost(req: Request, res: Response): Promise<void> {
        try {
            ...
            const updatedPost = await this.postService.updatePost(postId, req.user.id, content.trim());
            if (!updatedPost) {
