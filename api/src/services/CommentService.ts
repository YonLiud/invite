...
export class CommentService extends BaseService<Comment> {
    ...
    async createComment(authorId: number, postId: number, content: string): Promise<Comment | null> {
        ...
        const author = await AppDataSource.getRepository(Profile).findOneBy({ id: authorId });
    ...
    async getCommentsByAuthor(authorId: number, limit: number = 20): Promise<Comment[]> {
        return this.repository.find({
    ...
    async getCommentsWithPagination(postId: number, page: number = 1, limit: number = 20): Promise<{ items: Comment[], total: number, page: number, totalPages: number }> {
