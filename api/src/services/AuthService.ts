...
export class AuthService {
    ...
    async refreshAccessToken(refreshToken: string): Promise<{
        ...
        const isValid = await comparePassword(refreshToken, refreshTokenEntity.token_hash);
        if (!isValid) {
        ...
        refreshTokenEntity.expires_at = new Date(Date.now() + this.REFRESH_TOKEN_EXPIRES_DAYS * 24 * 60 * 60 * 1000);
